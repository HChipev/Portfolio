...
const Portfolios = () => {
  ...
  const [columnsDef] = useState([
    ...
    { headerName: "Project", field: "name", width: 100, resizable: true },
    {
  ...
  const handleDelete = async (id) => {
    await ApiService.deletePortfolio(id).catch((error) =>
      errorNotifications(
        error.response.data
          ? error.response.data.title ?? error.response.data
          : error.response.statusText
  ...
  const updateData = async (e) => {
    ...
    if (!modalData.id) {
      await ApiService.addPortfolio(modalData).catch((error) =>
        errorNotifications(
          error.response.data
            ? error.response.data.title ?? error.response.data
            : error.response.statusText
      ...
      await ApiService.updatePortfolio(modalData).catch((error) =>
        errorNotifications(
          error.response.data
            ? error.response.data.title ?? error.response.data
            : error.response.statusText
  ...
  return (
    <div className="flex flex-col w-full py-4">
      <div className="flex justify-between px-4">
        <h1 className="text-3xl text-amber-500 mb-4 px-4">Portfolios</h1>
        <button
      ...
      <ModalWrapper isOpen={isModalOpen} closeModal={closeModal}>
        <div className="flex flex-col items-center justify-center">
          <h1 className="text-2xl text-amber-500 mb-4">
            {modalData.id ? "Edit" : "Add"} Portfolio
          </h1>
